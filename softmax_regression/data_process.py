...
def load_mnist(file_dir, is_images='True'):
    ...
    if is_images:
        # Read images
        fmt_header = '>iiii'  # 大端法读取4个unsigned int32
        magic, num_images, num_rows, num_cols = struct.unpack_from(fmt_header, bin_data, 0)
    else:
        # Read labels
        fmt_header = '>ii' # MINST数据集的头部结构
        magic, num_images = struct.unpack_from(fmt_header, bin_data, 0)
        num_rows, num_cols = 1, 1
    data_size = num_images * num_rows * num_cols
    # 计算header大小偏移来读取后续的数据
...
# 我咋感觉能不能直接encoded, k = 10
def data_convert(x, y, m, k):
    x[x <= 40] = 0
    x[x > 40] = 1
    ont_hot_y = np.zeros((m,k)) # 60000*10大小的特征矩阵
    for t in np.arange(0,m):
        ont_hot_y[t,y[t]]=1
    ont_hot_y=ont_hot_y.T # 矩阵转置, (0,1,2...)*60000的矩阵
    return x, ont_hot_y
